      ...
      <main className="main-content">
        ...
        <div className="content-wrap" id="resumen">
          ...
          <section className={`panel dashboard-section ${activeSection === 'personas' ? '' : 'dashboard-section-hidden'}`} id="personas">
            ...
            {registeredUsers.length ? <div className="people-grid">{registeredUsers.map((user, index) => <div className="person-card person-card--interactive" key={user.id || `${user.dni}-${index}`}><button className="person-card-main" type="button" onClick={() => openUserVerification(user)}><div className="person-photo">{user.imagen_url ? <img src={user.imagen_url} alt={`Foto de ${user.nombre} ${user.apellido}`} /> : <span>{`${user.nombre?.[0] || ''}${user.apellido?.[0] || ''}`.toUpperCase()}</span>}</div><div><b>{user.nombre} {user.apellido}</b><span>{user.dni || 'Sin DNI'} · {user.email || 'Sin correo'}</span></div><Edit3 size={17} /></button><button className="icon-button person-card-delete" type="button" aria-label={`Eliminar a ${user.nombre} ${user.apellido}`} title="Eliminar persona" onClick={() => openUserVerification(user)}><Trash2 size={17} /></button></div>)}</div> : <div className="empty-state">Todavía no hay personas registradas. Usa “Reconocer rostro” para crear el primer perfil.</div>}
          </section>
