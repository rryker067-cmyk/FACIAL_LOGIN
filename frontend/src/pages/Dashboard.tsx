      ...
      <main className="main-content">
        ...
        <div className="content-wrap" id="resumen">
          ...
          <section className={`recognition-grid ${activeSection === 'reconocer' ? '' : 'dashboard-section-hidden'}`} id="reconocer">
            ...
            <div className="panel details-panel"><div className="panel-heading"><div><span className="section-kicker">PASO 02</span><h2>Datos personales</h2></div><span className="match-badge"><span /> Coincidencia lista</span></div>
              <div className={`recognition-live-status ${isCameraOpen ? 'is-active' : ''}`} role="status">
                <ScanFace size={18} />
                {scanCompleted ? 'Rostro detectado. Escaneo y cámara detenidos.' : isCameraOpen ? (liveScanPaused ? 'Rostro detectado. Escaneo detenido.' : isRecognizing ? 'Analizando el video en vivo...' : 'Análisis automático activo') : 'Activa la cámara para comenzar el análisis'}
              </div>
          ...
          <section className={`panel dashboard-section ${activeSection === 'personas' ? '' : 'dashboard-section-hidden'}`} id="personas">
            ...
            {filteredRegisteredUsers.length ? <div className="people-grid">{filteredRegisteredUsers.map((user, index) => <div className="person-card person-card--interactive" key={user.id || `${user.dni}-${index}`}><button className="person-card-main" type="button" onClick={() => openUserVerification(user)}><div className="person-photo">{user.imagen_url ? <img src={user.imagen_url} alt="" onError={(event) => { event.currentTarget.style.display = 'none'; event.currentTarget.parentElement?.classList.add('person-photo--fallback') }} /> : <span>{`${user.nombre?.[0] || ''}${user.apellido?.[0] || ''}`.toUpperCase()}</span>}</div><div><b>{user.nombre} {user.apellido}</b><span>{user.dni || 'Sin DNI'} · {user.email || 'Sin correo'}</span></div><Edit3 size={17} /></button><button className="icon-button person-card-delete" type="button" aria-label={`Eliminar a ${user.nombre} ${user.apellido}`} title="Eliminar persona" onClick={() => openUserVerification(user, 'delete')}><Trash2 size={17} /></button></div>)}</div> : <div className="empty-state">{globalSearch ? 'No hay personas que coincidan con la búsqueda.' : 'Todavía no hay personas registradas. Usa “Reconocer rostro” para crear el primer perfil.'}</div>}
          </section>
          ...
          {selectedUser && <div className="user-editor-backdrop" role="presentation" onClick={() => { closeVerifyCamera(); setSelectedUser(null) }}>
            <section className="user-editor panel" role="dialog" aria-modal="true" aria-labelledby="user-editor-title" onClick={(event) => event.stopPropagation()}>
              ...
              <div className="selected-user-summary"><div className="person-photo person-photo--large">{selectedUser.imagen_url ? <img src={selectedUser.imagen_url} alt="" onError={(event) => { event.currentTarget.style.display = 'none'; event.currentTarget.parentElement?.classList.add('person-photo--fallback') }} /> : <span>{`${selectedUser.nombre?.[0] || ''}${selectedUser.apellido?.[0] || ''}`.toUpperCase()}</span>}</div><div><b>{selectedUser.nombre} {selectedUser.apellido}</b><span>{userAction === 'delete' ? 'Confirma tu identidad para eliminar este perfil.' : 'Compara tu rostro con la foto registrada para continuar.'}</span></div></div>
              ...
              {selectedDocument && (
                <div style={{ marginBottom: 18, border: '1px solid rgba(148, 163, 184, 0.25)', borderRadius: 12, background: 'rgba(15, 23, 42, 0.92)', padding: 14 }}>
                  ...
                  {selectedDocument.type.startsWith('image/') || /\.(png|jpe?g|webp|gif|bmp|svg)$/i.test(selectedDocument.name) ? (
                    documentationPreviewUrl ? (
                      <img src={documentationPreviewUrl} alt="" onError={(event) => { event.currentTarget.style.display = 'none'; event.currentTarget.parentElement?.classList.add('document-preview--fallback') }} style={{ width: '100%', maxHeight: 360, objectFit: 'contain', borderRadius: 10, background: '#0b1220', display: 'block' }} />
                    ) : (
                      <div className="document-preview--fallback">La vista previa no está disponible. Puedes abrir o descargar el archivo.</div>
                    )
